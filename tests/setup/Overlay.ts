...
import { fixtures, tmp } from './common.js';
...
execSync(`npm exec make-index -- ${fixtures} --output ${index} --quiet`, { stdio: 'inherit' });

const server = createServer((request, response) => {
	const path = request.url == '/.index.json' ? index : join(fixtures, request.url?.slice(1) || '');
	try {
