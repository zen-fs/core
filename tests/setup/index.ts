...
import { configureSingle, CopyOnWrite, defaultContext, InMemory, InMemoryStore, Readonly, StoreFS } from '@zenfs/core';
import { S_IFDIR } from '@zenfs/core/constants';
import { copySync, data } from '../setup.ts';

copySync(data);

const index = (defaultContext.mounts.get('/') as StoreFS).createIndexSync();
