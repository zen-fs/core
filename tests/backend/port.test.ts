...
suite('Remote FS with resolveRemoteMount', () => {
	...
	test('Configuration', async () => {
		await configureSingle({ backend: Port, port: configPort, timeout: 1000 });
	});
...
await suite('FS with MessageChannel', () => {
	test('configuration', async () => {
		...
		await configureSingle({ backend: Port, port: channel.port1, disableAsyncCache: true, timeout: 1000 });
	});
...
await suite('Remote FS', () => {
	...
	test('Configuration', async () => {
		await configureSingle({ backend: Port, port: remotePort, timeout: 1000 });
	});
